...
def open_voice_stack(
    ...
    *,
    ...
    if device is None and isinstance(resolved_device, str):
        ...
        #
        # 退到默认设备之后麦克风**可能**是聋的（Windows 上一个被静音 / 被隐私设置拒绝的设备
        # 不报错），但那条路有专门的探测：开麦 4 秒后「全零输入」进运行日志（error 级）。
        # 「有一只可能听不见的麦克风」比「一只都没有」可诊断得多。
        #
        # **2026-09-04 删掉了这里原来那句「这台机器上默认就是那只峰值 0.00003 的阵列」。**
        # 使用者实测那只内置阵列**能正常完成语音唤醒**，而 0.00003 是安静房间的读数 ——
        # 峰值分不清「轻的语音」和「没有语音」（见 `core/audio/vad.py`），拿它给一只麦克风
        # 定性是拿一个答不了这个问题的量去回答它。
        #
