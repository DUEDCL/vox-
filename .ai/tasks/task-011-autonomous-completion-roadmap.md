...
7. ✅ **声纹门加固**（2026-08-24 完成，人类点名）：质量门（RMS 下限/削波比例）先于模型运行；暴力冷却（连续拒绝→冷却期拒答，注入时钟可测）；可选多窗口一致校验默认关闭待 REAL-MIC 调参；describe() 暴露 gate 配置与审计计数。tests/test_speaker_hardening.py 14 例，speaker 组 **44 passed**。启发式 ≠ 反录音回放，ADR 002 局限如实保留。
