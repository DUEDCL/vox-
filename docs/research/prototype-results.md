...
| ASR 往返 | 0（本机流式） | 3.1 s | **2.7 s** | 流式 WebSocket（`qwen3-asr-flash-realtime`，走 `/api-ws/v1/realtime`） |
| LLM 首字 | 5.0 s | 7.9 s | **1.8 s** | 已换 |
| TTS 整段 | 2.4 s | 3.6 s | **0.9 s** | 换 `-tts-flash` 还能再省 0.4 s，但那把嗓子要人听过 |
| **合计** | 8.6 s | **15.4 s** | **6.2 s** | ASR 流式做完约 4 s |

**剩下的一块是 ASR。** WS 客户端已经在（`core/ws.py`），所以那一步现在只是协议工作：
`qwen3-asr-flash-realtime` 在 `/api-ws/v1/inference` 上回 `ModelNotFound`，要走
`/api-ws/v1/realtime?model=…`（OpenAI Realtime 风格，事件名与这一族完全不同）。
