...
| 段 | 09-01 | 09-05（换端点前） | 换端点后 | 还能怎么砍 |
|---|---|---|---|---|
| VAD 端点 | 1.2 s | 0.8 s | 0.8 s | **不能**（这位说话人的短语间停顿 1.0–1.1 s，见上一节） |
| ASR | 0（本机流式） | **3.1 s** | 3.1 s | 流式接口（百炼有 `qwen3-omni-*-realtime` / `qwen3.5-omni-*-realtime`，WebSocket） |
| LLM 首字 | 5.0 s | 7.9 s | **1.8 s** | 已换 |
| TTS 首块 | 2.4 s | 2.4 s | 2.4 s | 流式接口（`qwen3-tts-flash-realtime`），或让 LLM 的第一句提前进合成 |
| **合计** | 8.6 s | **14.2 s** | **8.1 s** | 两块流式都做完约 2 s |

**没做的两块都要 WebSocket**，而标准库没有 WS 客户端 —— 那是一个新依赖或约 200 行自己写
（本仓库自己写过 AES-128，先例在 `core/channels/crypto.py`）。这一轮没做，理由是先把不需要
新协议的那 6.1 秒拿掉；判据与端点名记在这里，好让下一轮不必重新发现。
