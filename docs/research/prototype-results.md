...
| ASR 往返 | 0（本机流式） | 3.1 s | **2.7 s** | 流式 WebSocket（`qwen3-asr-flash-realtime`） |
| LLM 首字 | 5.0 s | 7.9 s | **1.8 s** | 已换 |
| TTS 整段到手 | 2.4 s | 3.6 s | 3.6 s | 收到第一块就播（省约 0.5 s，见上）；或流式 WS（`qwen3-tts-flash-realtime`） |
| **合计** | 8.6 s | **15.4 s** | **8.9 s** | 两块流式都做完约 3 s |

**没做的两块都要 WebSocket**，而标准库没有 WS 客户端 —— 那是一个新依赖或约 200 行自己写
（本仓库自己写过 AES-128，先例在 `core/channels/crypto.py`）。这一轮没做，理由是先把不需要
新协议的那 6.5 秒拿掉；判据与端点名记在这里，好让下一轮不必重新发现。
