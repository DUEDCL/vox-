...
> 最后更新:2026-08-23（VoiceRuntime、麦克风采集、DesktopBridge 与工具事件隐私加固）
...
| Python 测试 | **630 passed, 3 skipped**（全量 AUTO；DesktopBridge 专项 **33 passed**、采集专项 **36 passed**；无真实设备） |
...
| 真实透明窗口验收 | **未验证** — 发布阻塞项 |
| 版本控制 | 已推送到 `https://github.com/DUEDCL/vox-.git`;最近完成 DesktopBridge、Agent 失败路径与工具事件隐私加固，当前分支仍需独立审查 |
...
| 测试 | `tests/*.py` + `tests/integration/` | — | 633 collected（630 passed + 3 skipped）；DesktopBridge 专项 33 passed，采集专项覆盖启动回滚、回调隔离、ASR/KWS 恢复与幂等停止，见 [测试文档](testing.md) |
