# 交接文档 —— EvoX Voice Wake

> 交接时间：本会话结束时  ·  基线提交：`0cacde4`
> 验证基线：Python **599 passed, 2 skipped**（601 collected）· Rust **15 passed** · `npm run build` 通过
...
| 派发 / 路由 / 汇总 | `core/dispatch/` | AUTO+SIM 159 用例 | 真实 agent 跑一轮 |
...
**读代码的建议顺序**：`core/state.py`（6 态）→ `core/events.py`（信封）→ `evox_plugin/plugin.py`（门面）
→ `evox_plugin/runtime.py`（装配，看 `say()` 和 `pump()`）→ 再按需要下钻到 `core/dispatch/` 或 `core/audio/`。
...
```powershell
# 0) 环境自检（应当 599 passed, 2 skipped）
...
8. **控制台中文乱码不是缺陷** —— Windows 代码页显示问题，UTF-8 字节是对的，别去「修」。

## 7. 与本项目无关的目录（可直接删）

`github_program/`、`open-design-main.zip`、`.tmp-open-design*/` 是从 GitHub 拉的临时参考物，
**不是本项目代码**，已确认可不理会。`models/` 约 597 MB（gitignore 内），
其中 `kws.tar.bz2` / `tts.tar.bz2` / `asr.tar.bz2` 三个归档是解压完可删的。
