...
| 3dspeaker_speech_eres2net_base_sv_zh-cn_3dspeaker_16k.onnx | https://github.com/k2-fsa/sherpa-onnx/releases/download/speaker-recongition-models/ (tag typo `recongition` is upstream's) | 3D-Speaker upstream: Apache-2.0 | **superseded 2026-08-29** by CAM++ (still on disk) | SHA-256 `1a331345f04805badbb495c775a6ddffcdd1a732567d5ec8b3d5749e3c7a5e4b`; 39,593,761 bytes; embedding dim 512 |
| 3dspeaker_speech_campplus_sv_zh-cn_16k-common.onnx | same release tag | 3D-Speaker upstream: Apache-2.0 | **adopted 2026-08-29** (speaker gate default) | SHA-256 `f682b514c05d947ee3fa91cd6ec6c5c7543479a128373fa29b1faedccd21fd11`; 28,281,138 bytes; embedding dim **192**. Chosen on measured discriminability over the 7 bundled real-voice wavs, not on upstream claims: same-speaker minimum **0.833** vs ERes2Net's 0.736, different-speaker maximum 0.396 vs 0.370, so the usable **gap widened from +0.366 to +0.437** while the file shrank 10.8 MB and the vector got 2.7× shorter. Swapping invalidates every existing enrolment (512-dim vectors are meaningless to a 192-dim model); `_restore()` reports them in `stale_profiles` rather than silently behaving like "nobody enrolled" |
