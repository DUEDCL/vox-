...
class SqliteMemoryStore:
    ...
    def __init__(self, db_path: str | Path = ":memory:") -> None:
        ...
        #: Serialises access so one connection can be shared across threads.
        #: Needed because the callers are genuinely multi-threaded: the console's
        #: HTTP handler runs on a worker thread, the turn pump on another, and the
        #: audio callback on a third. Without this, sqlite3's default
        #: ``check_same_thread=True`` raises ``ProgrammingError`` on whichever
        #: thread did not open the connection -- and it does so *after* the first
        #: query has already succeeded elsewhere, which reads as an intermittent
        #: failure rather than a design mistake.
    ...
    def connection(self) -> sqlite3.Connection:
        with self._lock:
            if self._conn is None:
                ...
                # ``check_same_thread=False`` is safe only because every statement
                # below goes through ``self._lock``. SQLite itself is built for
                # this (serialized threading mode); what it will not tolerate is
                # two threads inside one transaction at once.
