...
#: 只是「换了个更能干、更慢的后端答同一句话」—— 代价是时间，不是一次副作用。所以这里
#: 用 ``search()`` 而不是 ``\\A``：「这个函数你帮我改一下」的动词在句尾，而它确实是要
#: 改代码。反方向（漏判）才是真损失：一句「跑一下测试」落到裸端点上，换回来的是一段
#: 它其实执行不了的说明。
#:
#: **那个「代价」原来写的是「几秒」，而 2026-09-05 量出来是 14–28 秒。** 换端点之后
#: 两边的差距拉开了：relay（qwen-plus）首字 1.8 s，而 `claude` CLI 那一轮实测第一声
#: 14733–27969 ms —— 大头是 CLI 自己的启动开销，和这一层无关但要算进这笔账。
#: 所以「报错 / 调试」这一族里**纯提问**的那些（「这个报错是什么意思」）现在要付 8–15 倍
#: 的时间，而它们本来 relay 答得了。方向仍然是对的（漏判会得到一段执行不了的说明），
#: 但收窄这一族现在有了实测依据 —— 收之前先想清楚「帮我看看这个报错」怎么留在 CLI 那一侧。
_CODE_PATTERNS = (
